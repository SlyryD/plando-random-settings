...
# Draw from the geometric distribution (the number of successful coin flips before the first failure up to k)
def draw_from_geometric_distribution(k):
    result = 0
    for _ in range(k):
...
# Randomize starting pool
def populate_starting_pool(pool):
    shuffled_pool = random.sample(list(pool), len(pool))
    num = draw_from_geometric_distribution(len(pool))
    return shuffled_pool[:num]
...
def main():
    ...
    if ALLOW_MASTERQUEST:
        ...
        random_settings["mq_dungeons"] = draw_from_geometric_distribution(12)

    # Choose the number of starting hearts
    random_settings["starting_hearts"] = 3 + draw_from_geometric_distribution(17)

    # Choose the number of big poes
    random_settings["big_poe_count_random"] = False
    random_settings["big_poe_count"] = 1 + draw_from_geometric_distribution(9)
